...
class PortfolioUserActor(remoteDictionary: ActorRef) extends Actor with ActorLogging {
    ...
    case UserSetDataMessage(userList) =>
      {
        ...
        log.info(s"---> TAMAÃ‘O DE LA LISTA DE USUARIOS: " + nrUsers)
    ...
    case Fact(userId, fact /*: List[NumCodeFact]*/ ) =>
      {
        ...
        if (nrOfResults == nrUsers) {
          log.info(s"---> LOS FACTORIALES DE TODOS LOS USUARIOS (${nrUsers}) YA ESTÃ�N COMPLETOS: ${facts}")
  ...
  We donâ€™t have to specify our own supervisor strategy in each and every actor.
